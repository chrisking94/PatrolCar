...
void NVIC_init(void)
{
	NVIC_SetVectorTable(NVIC_VectTab_FLASH,0x0);//或NVIC_VectTab_RAM
	...
	//NVIC_new(SPI1_IRQn,1,2);
	//NVIC_new(SPI2_IRQn,1,2);
	//NVIC_new(DMA1_Channel3_IRQn,1,1);//SPI1.DMA
	//NVIC_new(DMA1_Channel1_IRQn,1,1);//ADC1.DMA
	#ifdef IR_CTRL_
		NVIC_new(TIM3_IRQn,1,3);//TIM3中断,红外
